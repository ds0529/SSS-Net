...
class ConfigS3DIS:
    ...
    noise_init = 3.5  # noise initial parameter
    max_epoch = 50  # 100 # maximum epoch during training
...
class DataProcessing:
    ...
    @staticmethod
    def data_aug_rg(xyz, color, labels, rg, rgRGB, rg_combine, idx, num_out):
        num_in = len(xyz)
        dup = np.random.choice(num_in, num_out - num_in)
        xyz_dup = xyz[dup, ...]
        xyz_aug = np.concatenate([xyz, xyz_dup], 0)
        ...
        label_aug = labels[idx_dup]
        rg_aug = rg[idx_dup]
        rgRGB_aug = rgRGB[idx_dup]
        rg_combine_aug = rg_combine[idx_dup]
        return xyz_aug, color_aug, idx_aug, label_aug, rg_aug, rgRGB_aug, rg_combine_aug
