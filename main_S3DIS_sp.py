...
from tester_S3DIS_sp import ModelTester
...
class S3DIS:
    def __init__(self, test_area_idx):
        self.name = 'S3DIS'
        self.test_area = test_area_idx
        self.path = '/home/newdisk/dengshuang/S3DIS/Stanford3dDataset_v1.2_Aligned_Version'
        ...
        self.fs_files = [line.rstrip() for line in open('utils/S3DIS_fs_samples_' + str(cfg.fs_ratio) + '.txt')]
        ...
        self.input_trees = {'training_fs': [], 'training_all': [], 'validation': []}  # 子点云树，这些都是子点云
    ...
    def load_sub_sampled_clouds(self, sub_grid_size):
        ...
        for i, file_path in enumerate(self.all_files):
            ...
            elif cloud_name + '.ply' in self.fs_files:
            ...
            # region growing
            self.rg_path = join(self.path, 'input_{:.3f}'.format(sub_grid_size), 'region_growing')
            self.rgRGB_path = join(self.path, 'input_{:.3f}'.format(sub_grid_size), 'region_growingRGB')
            self.rg_combine_path = join(self.path, 'input_{:.3f}'.format(sub_grid_size), 'region_growing_combine')
            rg_file = join(self.rg_path, '{:s}.txt'.format(cloud_name))  # 子点云
            rgRGB_file = join(self.rgRGB_path, '{:s}.txt'.format(cloud_name))  # 子点云
            rg_combine_file = join(self.rg_combine_path, '{:s}.txt'.format(cloud_name))  # 子点云
    ...
    def get_batch_gen(self, split):
        if split == 'training':
            ...
            # fs
            self.possibility['training_fs'] = []  # 点的概率
            self.min_possibility['training_fs'] = []  # 点云的概率
            # Random initialize
            for i, tree in enumerate(self.input_colors['training_fs']):  # 一开始概率随机
            ...
            # all
            self.possibility['training_all'] = []  # 点的概率
            self.min_possibility['training_all'] = []  # 点云的概率
            # Random initialize
            for i, tree in enumerate(self.input_colors['training_all']):  # 一开始概率随机
        ...
        def spatially_regular_gen_training():
            ...
            for i in range(num_per_epoch):  # 每个epoch输入的点云数目
                ...
                cloud_idx = int(np.argmin(self.min_possibility[split_tmp]))  # 概率最小点云
                fs_switch = 0
                if split_tmp == 'training_fs':
                    fs_switch = 1

                # choose the point with the minimum of possibility in the cloud as query point
                point_ind = np.argmin(self.possibility[split_tmp][cloud_idx])  # 概率最小点

                # Get all points within the cloud from tree structure
                points = np.array(self.input_trees[split_tmp][cloud_idx].data, copy=False)  # 从树结构中找到子点云坐标
                ...
                if len(points) < cfg.num_points:
                    ...
                    queried_idx = self.input_trees[split_tmp][cloud_idx].query(pick_point, k=len(points))[1][0]  # 在中心点周围找最近邻作为输入
                else:
                ...
                self.possibility[split_tmp][cloud_idx][queried_idx] += delta  # 离得越远，概率加的越少
                ...
                if True:
                    yield (queried_pc_xyz.astype(np.float32),
                           ...
                           np.array([fs_switch], dtype=np.int32))  # 当前点云是否为全监督
