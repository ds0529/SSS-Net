...
class ModelTester:
    ...
    def test(self, model, dataset, num_votes=100):
        ...
        while last_min < num_votes:
            try:
                flat_inputs = self.sess.run(model.flat_inputs)
                # get sp inputs
                sp_labels_rg, sp_labels_rgRGB, sp_labels_rg_combine = self.sess.run(
                    [model.inputs['rg'], model.inputs['rgRGB'], model.inputs['rg_combine']],
                    model.get_feed_dict(flat_inputs, False))
                # get spagg
                if model.sp_agg_switch or model.sp_loss_switch:
                    if model.sp_switch == 0:
                        batch_sp_agg_indices = model.get_spagg(sp_labels_rg)
                    elif model.sp_switch == 1:
                        batch_sp_agg_indices = model.get_spagg(sp_labels_rgRGB)
                    else:
                        batch_sp_agg_indices = model.get_spagg(sp_labels_rg_combine)
                ops = (self.prob_logits,
                       model.labels,
                       model.inputs['input_inds'],
                       model.inputs['cloud_inds']
                       )  # 这是子点云的结果
                test_feed_dictionary = model.get_feed_dict(flat_inputs, False)
                if model.sp_agg_switch or model.sp_loss_switch:
                    test_feed_dictionary[model.batch_sp_agg_indices_input] = batch_sp_agg_indices
                stacked_probs, stacked_labels, point_idx, cloud_idx = self.sess.run(ops, test_feed_dictionary)
