...
def gen_pickle(split = "val", root = "DataSet/Scannet_v2"):
    if split == 'test':
        root_data = root + "/test"
        root_label = root + "/scans_labels/scans/test"
    elif split == 'train':
        root_data = root + "/train"
        root_label = root + "/scans_labels/scans/train"
    else:
        root_data = root + "/val"
        root_label = root + "/scans_labels/scans/val"
    ...
    for i in range(len(scene_id)): #len(scene_id)
        print('process...', i)
        scene_namergb = os.path.join(root_data, scene_id[i]+'_vh_clean_2.ply')
        ...
        if split != 'test':
            scene_name = os.path.join(root_label, scene_id[i]+'_vh_clean_2.labels.ply')
    ...
    pickle_out = open(os.path.join(root, "scannet_%s_rgb21c_pointid.pickle"%(split)),"wb")
...
if __name__ =='__main__':
    root = '/home/newdisk/dengshuang/scannet/scans'
    # root = '/share/group/datanlpr9/data/scannet/scans'
